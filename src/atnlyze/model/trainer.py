...
def main():
    ...
    vectorizer = TfidfVectorizer(
        ngram_range=(1, 2),
        max_features=10000,  # increased from 5000; dataset is larger now
        stop_words=None,  # do NOT use stop_words; attack keywords matter
    )
    ...
    model = LogisticRegression(
        max_iter=1000,
        class_weight="balanced",  # safety net for residual class imbalance
        C=1.0,  # default regularization; tune if val F1 drops
    )
